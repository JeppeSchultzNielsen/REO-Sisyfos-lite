...
def plot_energy_transfer(df, node, period, EENS_sur):
    ...
    # Laver et et stacked bar chart af import og eksport værdierne for hvert time
    fig, ax = plt.subplots()
    import_colors = ['#1F4F6F', '#095256', '#2C685F', '#38814F', '#05692B', '#008000', '#66B048', '#99C86C', '#99A86C', '#CBDF90', '#A6A679', '#94896D', '#816C61', '#BEC5AD']  # green shades
    export_colors = ['#533745', '#7E3F4C', '#840000', '#FF0000', '#A94652', '#FF555E', '#FFAABB', '#D2886F', '#A46523', '#CD7C12', '#F59300', '#E09D00', '#FFCC00', '#FFFAA5']  # red shades
    import_handles = []
    export_handles = []
    for i, node_before in enumerate(import_values.columns):
        import_handle = ax.bar(import_values.index, import_values[node_before], bottom=import_values.iloc[:, :i].sum(axis=1), label=node_before + ' import', color=import_colors[i])
    ...
    for i, node_after in enumerate(export_values.columns):
        ...
        export_handles.append(export_handle)
    ax.set_xlabel('Time')
    ax.set_ylabel('Energi transfer (MWh)')
    ax.set_title(f"Energi import og eksport ({start_hour} til {end_hour}) for {node}")
    ax.legend(handles=import_handles + export_handles, bbox_to_anchor=(1.025, 0.5), loc='center left', borderaxespad=0., ncol=2)
    plt.show()
    print(f'Total surplus: {total_surplus:,.2f}. \nTotal EENS: {total_EENS:,.2f}. ')
